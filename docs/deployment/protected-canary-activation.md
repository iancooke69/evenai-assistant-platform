...
- one live disabled Worker version at 100%;
- `ENABLE_PUBLIC_ASSISTANT=false` and an empty live origin list before activation;
...
Any failure after the plan is created triggers an immediate deployment of the stable disabled version at 100%. The workflow verifies that restoration and writes `canary-rollback-evidence`.
