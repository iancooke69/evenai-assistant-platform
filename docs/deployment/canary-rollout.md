...
- an explicit initial exposure percentage;
- an explicit observation period.

## Promotion

A rollout may use an initial canary stage followed by full exposure. Promotion is permitted only when every required production check continues to pass throughout the observation window.
